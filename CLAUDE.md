...
- `api/` — routers (health, auth: register/login, me, admin: ping, catalog: public GET /catalog + /catalog/videos/{id}, events: POST /events/heartbeat|complete|rating|watch-party, all auth) + `deps.py` SessionDep/CurrentUserDep/AdminDep (use `Annotated[..., Depends]`, never `= Depends()` — ruff B008; missing/bad token → 401, non-admin → 403). `main.py` — `create_app()` factory, lifespan does schema+seed (challenges+catalog)+admin bootstrap (ADMIN_USERNAME/_PASSWORD env); module-level `app` for uvicorn.
