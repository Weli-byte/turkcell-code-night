...
- `services/` — state_builder (watch_events → engine DailyUserState: today+7d sums, distinct genres via video join, consecutive-day streak; integer division), live_evaluator (per-event: engine evaluate+select_reward → reward_events insert (UNIQUE(user_id,reward_date) = one reward/day) → ledger append (source_ref=reward:<date>) → badge thresholds → deduped notifications; returns LiveEvaluationResult), notifier (thread-safe in-process NotificationBroker for SSE, single-process — Redis pub/sub in backlog), daily_batch (replays evaluate_user_live for all users in user_id order, writes RunRecord with JSON summary; can only fill gaps, never contradicts live — all writes idempotent), simulator (TrafficSimulator: persona bots binge/gundelik/elestirmen through the REAL ingestion path — EventRepository guards + evaluate_user_live apply; seeded random.Random for reproducible traffic (engine no-random rule untouched); asyncio tick loop, admin start/stop, app.state.simulator), explain (maps DB rows → engine domain models keyed by USERNAME not uuid, calls engine explain_user_query, then LLM adapter rephrase-or-fallback; POST /explain auth endpoint, question 3-300 chars, user always from token), scheduler (stdlib asyncio DailyJobScheduler, NOT APScheduler — no dep, typed; runs batch daily at batch_hour:batch_minute UTC, default 23:55; SCHEDULER_ENABLED=0 to disable, tests disable it).
- `security.py` — bcrypt hash/verify + HS256 JWT create/decode (TokenError, injectable `now`). No FastAPI imports.
- `api/` — routers (health, auth: register/login, me: GET /me|/me/points|/me/badges|/me/challenges (live progress via engine parser)|/me/notifications, admin: ping + GET/POST /admin/challenges + PUT /admin/challenges/{id} (condition pre-validated via services/condition_validation.py — whitelist derived from a zero DailyUserState.to_rule_context(), invalid → 422; partial update; edits affect future evaluations only) + GET /admin/users (with point totals) + GET /admin/simulator + POST /admin/simulator/start|stop (409 when already running; bot_count 1-50, tick_seconds 0.5-60) + GET /admin/runs + POST /admin/batch-run, leaderboard: GET /leaderboard (auth, live from ledger, tie-break alphabetical username, limit param, is_bot flag), catalog: public GET /catalog + /catalog/videos/{id}, events: POST /events/heartbeat|complete|rating|watch-party — all auth, response includes reward+new_badges from live evaluation, sse: GET /sse/notifications?token= (query token, EventSource can't set headers)) + `deps.py` SessionDep/CurrentUserDep/AdminDep (use `Annotated[..., Depends]`, never `= Depends()` — ruff B008; missing/bad token → 401, non-admin → 403). `main.py` — `create_app()` factory, lifespan does schema+seed (challenges+catalog)+admin bootstrap (ADMIN_USERNAME/_PASSWORD env); module-level `app` for uvicorn.
