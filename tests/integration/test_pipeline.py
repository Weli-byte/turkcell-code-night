...
def test_pipeline_integration_runs_successfully_and_is_idempotent(
    tmp_path: Path,
) -> None:
    ...
    assert (
        summary.leaderboard_size == 1
    )  # Only U1 has points (U2 doesn't trigger rewards)
    ...
    # Daily Watcher (C-01) is suppressed because Episode Finisher (C-02)
    # has priority 3 < 5
    ...
    # 2. Second Run: Run the same pipeline again on the same output
    # directory to test idempotency
    summary_run2 = run_pipeline(ctx)

    assert summary_run2.total_rewards_generated == 1  # Generates reward candidate again
    assert (
        summary_run2.total_ledger_entries == 1
    )  # Ledger should NOT duplicate the entry
    assert (
        summary_run2.total_notifications_created == 1
